...
本資料は、作成したLTS解析アプリケーションで用いている理論式と計算の流れを確認するためのノートである。
...
である。
アプリでは、この値を \texttt{TS断面積} の初期値としているが、ユーザーが自由に変更できる。
...
アプリでは、\texttt{ラマン有効断面積} の初期値として $3.82\times10^{-34}\ \mathrm{m^2}$ を入れている。
...
=
\sigma_{\mathrm{pixel}}
\left(\frac{\mathrm{nm}}{\mathrm{pixel}}\right)
\end{equation}
...
となる。
アプリの \texttt{ラマン圧力依存性} タブでは、各SPEファイルの測定圧力を入力し、ラマンピーク高さまたはガウス面積を圧力に対してプロットする。
